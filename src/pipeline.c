...
pipeline_builder *pipeline_builder_new(device *device) {
    ...
    builder->vertex_input_attributes = darray_create(VkVertexInputAttributeDescription);
    builder->vertex_input_bindings = darray_create(VkVertexInputBindingDescription);
...
void pipeline_builder_set_shaders(pipeline_builder *builder,
    ...
    if (builder->vertex_shader_module != VK_NULL_HANDLE) {
        vkDestroyShaderModule(device_get_handle(builder->device),
                              builder->vertex_shader_module,
                              NULL);
    }
    builder->vertex_shader_module = create_shader_module(device_get_handle(builder->device),
                                                         vertex_shader_code,
                                                         vertex_shader_size);
    ...
    builder->fragment_shader_module = create_shader_module(device_get_handle(builder->device),
...
pipeline *pipeline_builder_build(pipeline_builder *builder, VkRenderPass render_pass) {
    pipeline *pipeline = malloc(sizeof(struct pipeline_t));
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(device_get_handle(builder->device),
                                         &layout_info,
                                         NULL,
    ...
    VK_CHECK(vkCreatePipelineLayout(device_get_handle(builder->device),
    ...
    VK_CHECK(vkCreateGraphicsPipelines(device_get_handle(builder->device),
                                       VK_NULL_HANDLE,
                                       1,
                                       &create_info,
                                       NULL,
    ...
    device_create_buffer(builder->device,
                         builder->ubo_size,
                         VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                         &pipeline->uniform_buffer,
                         &pipeline->uniform_buffer_memory);
    vkMapMemory(device_get_handle(builder->device),
                pipeline->uniform_buffer_memory,
                0,
                builder->ubo_size,
                0,
    ...
    VK_CHECK(vkCreateDescriptorPool(device_get_handle(builder->device),
                                    &pool_info,
                                    NULL,
    ...
    VK_CHECK(vkAllocateDescriptorSets(device_get_handle(builder->device),
                                      &alloc_info,
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        vkUpdateDescriptorSets(device_get_handle(builder->device),
    ...
    vkDestroyShaderModule(device_get_handle(builder->device), builder->vertex_shader_module, NULL);
    vkDestroyShaderModule(device_get_handle(builder->device),
                          builder->fragment_shader_module,
                          NULL);
...
void pipeline_destroy(pipeline **pipeline, device *device) {
    vkDestroyDescriptorPool(device_get_handle(device), (*pipeline)->descriptor_pool, NULL);
    vkDestroyDescriptorSetLayout(device_get_handle(device),
                                 (*pipeline)->global_descriptor_set_layout,
                                 NULL);

    vkDestroyPipeline(device_get_handle(device), (*pipeline)->handle, NULL);
    vkDestroyPipelineLayout(device_get_handle(device), (*pipeline)->layout, NULL);
